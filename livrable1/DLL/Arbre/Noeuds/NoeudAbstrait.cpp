...
void NoeudAbstrait::calculBoiteEnglobante()
{
	...
	double longueur = abs(boite.coinMax[0] - boite.coinMin[0]) * scale_[0] * multiplicateurScale_;
	double hauteur = abs(boite.coinMax[1] - boite.coinMin[1]) * scale_[1] * multiplicateurScale_;
	...
	float radAngle = 3.14159265358979323846 * angleRotation / 180;

	boiteEnglobante.coinHG[0] = boiteEnglobante.coinHG[0] * std::cos(radAngle) - boiteEnglobante.coinHG[1] * std::sin(radAngle);
	boiteEnglobante.coinHG[1] = boiteEnglobante.coinHG[0] * std::sin(radAngle) + boiteEnglobante.coinHG[1] * std::cos(radAngle);

	boiteEnglobante.coinBG[0] = boiteEnglobante.coinBG[0] * std::cos(radAngle) - boiteEnglobante.coinBG[1] * std::sin(radAngle);
	boiteEnglobante.coinBG[1] = boiteEnglobante.coinBG[0] * std::sin(radAngle) + boiteEnglobante.coinBG[1] * std::cos(radAngle);

	boiteEnglobante.coinHD[0] = boiteEnglobante.coinHD[0] * std::cos(radAngle) - boiteEnglobante.coinHD[1] * std::sin(radAngle);
	boiteEnglobante.coinHD[1] = boiteEnglobante.coinHD[0] * std::sin(radAngle) + boiteEnglobante.coinHD[1] * std::cos(radAngle);

	boiteEnglobante.coinBD[0] = boiteEnglobante.coinBD[0] * std::cos(radAngle) - boiteEnglobante.coinBD[1] * std::sin(radAngle);
	boiteEnglobante.coinBD[1] = boiteEnglobante.coinBD[0] * std::sin(radAngle) + boiteEnglobante.coinBD[1] * std::cos(radAngle);
}
...
void NoeudAbstrait::dessinerBoiteEnglobante() const
{
	...
	if (estSelectionne())
	{
		...
		//glScalef(multiplicateurScale_ * 1.1 * scale_[0], 1.1 * scale_[1], 1.1 * scale_[2]);
...
void NoeudAbstrait::rotation(double angle, glm::dvec3 centre)
{
	...
	{
		float distX = positionRelative_[0] - centre[0];
		float distY = positionRelative_[1] - centre[1];

		float radAngle = 3.14159265358979323846 * angle / 180;

		float posX = distX * std::cos(radAngle) - distY * std::sin(radAngle) + centre[0];
		float posY = distX * std::sin(radAngle) + distY * std::cos(radAngle) + centre[1];
