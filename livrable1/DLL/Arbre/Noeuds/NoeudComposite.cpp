...
void NoeudComposite::effacerSelection()
{
	...
	for (conteneur_enfants::iterator it{ enfants_.begin() };
		...
		{ 
			...
			NoeudPortail* portail1;
			int id;

			if (enfant->obtenirType() == "portail")
			{
				portail1 = ((NoeudPortail*) enfant);
				id = portail1->obtenirId();
				enfants_.erase(it);
				
				for (conteneur_enfants::iterator it2{ enfants_.begin() };
					it2 != enfants_.end();)
				{
					NoeudAbstrait* enfant2{ (*it2) };
					NoeudPortail* portail2;
					if (enfant2->obtenirType() == "portail" && ((NoeudPortail*)enfant2)->obtenirId() == id)
					{
							enfants_.erase(it2);
							delete enfant2;
							it2 = enfants_.begin();				
