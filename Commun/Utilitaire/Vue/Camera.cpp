...
namespace vue {
		...
		)
	{
		if (empecheInversion)
		{
			if (position_[1] + rotationY > 85 || position_[1] + rotationY <0)
			{
				empecheInversion = false;
			}
			else
			{
	...
	void Camera::positionner() const
	{
		...
		//gluLookAt(dist * sin(phi)*sin(theta), dist * cos(phi), dist * sin(phi)*cos(theta), 
		//	pointVise_[0], pointVise_[1], pointVise_[2],
		//	directionHaut_[0], directionHaut_[1], directionHaut_[2]);

		glTranslated(0, 0, -position_[2]);
		glRotated(position_[1], -1, 0, 0);
		glRotated(position_[0], 0, 0, 1);
	}
